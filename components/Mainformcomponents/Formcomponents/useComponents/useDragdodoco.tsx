...
export default function useDragdodoco(
  type: DragType,
  id: string,
  isSide: boolean,
  dispatch: React.Dispatch<Action>,
  statusdispatch: React.Dispatch<CharArtWepAction>
) {
  const [{ isDragging }, drag] = useDrag(() => ({
    ...
    item: { name: type, side: isSide, id },
      ...
      if (item && dropResult && isSide && type !== 'culc') {
      ...
      if (item && dropResult && !isSide && item.name !== 'culc') {
        if (item.id !== dropResult.id) {
          ...
          dispatch({ action: 'siblingNode', id: dropResult.id, type: item.name });
          statusdispatch({ action: 'copycharartwepaction', toid: 'maxid', fromid: item.id });
        }
      }
    },
