  ...
  const typeelement = types.map((type0) => {
    const typeelement0 = type0.children.map((type1) => {
      const typeelement1 = type1.children.map((type2) => {
        if (type2.type === 'wep') {
          return <Weaponform g={2} key={type2.id} onDelete={() => dispatch({ action: 'deleteNode', id: type2.id })} />;
        } else {
          return <Artifactform g={2} key={type2.id} onDelete={() => dispatch({ action: 'deleteNode', id: type2.id })} dispatch={dispatch} />;
        }
      });

      if (type1.type === 'wep') {
        return (
          <Weaponform g={1} key={type1.id} onDelete={() => dispatch({ action: 'deleteNode', id: type1.id })}>
            {typeelement1}
          </Weaponform>
        );
      } else {
        return (
          <Artifactform g={1} key={type1.id} onDelete={() => dispatch({ action: 'deleteNode', id: type1.id })}>
            {typeelement1}
          </Artifactform>
        );
      }
